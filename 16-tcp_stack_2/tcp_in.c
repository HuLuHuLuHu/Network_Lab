...
void tcp_sock_listen_dequeue(struct tcp_sock *tsk) {
  if (tsk->parent) {
    if (!list_empty(&tsk->parent->listen_queue)) {
      ...
      list_for_each_entry(entry, &(tsk->parent->listen_queue),
                          list) if (entry == tsk)
...
void tcp_process(struct tcp_sock *tsk, struct tcp_cb *cb, char *packet) {
  ...
  switch (tsk->state) {
    ...
    case TCP_ESTABLISHED:
      // fprintf(stdout, "In TCP_ESTABLISHED state:\nflag: %d\n", cb->flags);
      if (cb->flags & TCP_FIN) {
        tsk->rcv_nxt = cb->seq + 1;
        tcp_send_control_packet(tsk, TCP_ACK);
        tcp_set_state(tsk, TCP_CLOSE_WAIT);
        // fix a small bug in order to close
        sleep(2);
        tcp_sock_close(tsk);
      } else if (cb->flags & TCP_PSH) {
        tsk->rcv_nxt = cb->seq + 1;
        // fprintf(stdout, "receive tcp payload start to write to buffer.\n");
