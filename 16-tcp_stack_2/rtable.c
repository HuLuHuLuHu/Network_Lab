...
void init_rtable() { init_list_head(&rtable); }
...
void add_rt_entry(rt_entry_t *entry) { list_add_tail(&entry->list, &rtable); }
...
void print_rtable() {
  ...
  list_for_each_entry(entry, &rtable, list) {
    fprintf(stdout, IP_FMT "\t" IP_FMT "\t" IP_FMT "\t%s\n",
            HOST_IP_FMT_STR(entry->dest), HOST_IP_FMT_STR(entry->mask),
            HOST_IP_FMT_STR(entry->gw), entry->if_name);
  }
  fprintf(stdout, "--------------------------------------\n");
}
