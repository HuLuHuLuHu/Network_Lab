...
bool compare_port_priority(stp_port_t *p1,stp_port_t *p2){
	if(p1->designated_root > p2->designated_root || 
	(p1->designated_root == p2->designated_root && p1->designated_cost > p2->designated_cost) ||
	(p1->designated_root == p2->designated_root && p1->designated_cost == p2->designated_cost && 
	p1->designated_switch > p2->designated_switch) ||
	(p1->designated_root == p2->designated_root && p1->designated_cost == p2->designated_cost && 
	p1->designated_switch == p2->designated_switch && p1->designated_port > p2->designated_port))
		return false;
	else 
		return true;
...
bool compare_refresh_config(struct stp_config *receive,stp_port_t *p){
	if(
	// condition 1
	p->designated_root > ntohll(receive->root_id) ||
	// condition 2
	(p->designated_root ==  ntohll(receive->root_id) && p->designated_cost >  ntohl(receive ->root_path_cost)) || 
	// condition 3
	(p->designated_root ==  ntohll(receive->root_id) && p->designated_cost ==  ntohl(receive ->root_path_cost) && 
	p->designated_switch >  ntohll(receive->switch_id)) ||
	// condition 4
	(p->designated_root == ntohll(receive->root_id) && p->designated_cost ==  ntohl(receive ->root_path_cost) && 
	p->designated_switch ==  ntohll(receive->switch_id) && p->designated_port >  ntohs(receive->port_id)) ){
		//refresh the port
		p->designated_root =  ntohll(receive->root_id);
		p->designated_cost =  ntohl(receive->root_path_cost);
		p->designated_port =  ntohs(receive->port_id); 
		p->designated_switch =  ntohll(receive->switch_id);

	stp_port_t *root = p;
	for(int i=0;i<stp->nports;i++)
	if(!stp_port_is_designated(&(stp->ports[i])) && compare_port_priority(&(stp->ports[i]),root))
		root = &(stp->ports[i]);
		//refresh the switch
		...
		struct stp_config *config)
{
	...
	bool renew = compare_refresh_config(config,p);	
	stp_send_config(stp);
}
