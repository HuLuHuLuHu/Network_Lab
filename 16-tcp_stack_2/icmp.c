...
void icmp_send_packet(const char *in_pkt, int len, u8 type, u8 code) {
  ...
  struct icmphdr *out_icmp =
      (struct icmphdr *)((char *)out_ip + IP_BASE_HDR_SIZE);
  out_icmp->type = type;
  out_icmp->code = code;
  ...
  if (type == ICMP_ECHOREPLY) {
    ...
    memcpy((char *)out_icmp + ICMP_HDR_SIZE, (char *)in_ip,
           icmp_len - ICMP_HDR_SIZE);
  }
  ...
  if (type == ICMP_ECHOREPLY) {
    ...
    if (!entry) {
      log(ERROR,
          "could not find route entry when sending icmp packet, impossible.");
      free(out_pkt);
      return;
    }
