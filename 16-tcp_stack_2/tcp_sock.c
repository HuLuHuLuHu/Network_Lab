...
int tcp_hash(struct tcp_sock *tsk) {
  ...
  if (tsk->state == TCP_LISTEN) {
    // log(ERROR, "hash into listen table: %d", tsk->sk_sport);
    hash = tcp_hash_function(0, 0, tsk->sk_sport, 0);
    list = &(tcp_listen_sock_table[hash]);
  } else {
    // log(ERROR, "hash into established table.");
...
int tcp_sock_connect(struct tcp_sock *tsk, struct sock_addr *skaddr) {
  ...
  // 4. It is handled in tcp_in.c: tcp_state_syn_sent()
...
struct tcp_sock *tcp_sock_accept(struct tcp_sock *tsk) {
  // fprintf(stdout, "TODO: implement this function please.\n");
  if (list_empty(&tsk->accept_queue)) sleep_on(tsk->wait_accept);
...
int tcp_sock_read(struct tcp_sock *tsk, char *buf, int len) {
  if (ring_buffer_empty(tsk->rcv_buf)) sleep_on(tsk->wait_recv);
