...
// XXX: All the functions in this file should be treated as a blackbox. You do
// not
// need to understand how it works, but only trust it will process like the
// function
// name indicates.
...
static iface_info_t *if_name_to_iface(const char *if_name) {
  ...
  list_for_each_entry(iface, &instance->iface_list, list) {
    if (strcmp(iface->name, if_name) == 0) return iface;
  }

  fprintf(stderr,
          "Could not find the desired interface "
...
static int parse_routing_info(char *buf, int len) {
  ...
  for (struct nlmsghdr *nlp = (struct nlmsghdr *)buf; NLMSG_OK(nlp, len);
       nlp = NLMSG_NEXT(nlp, len)) {
    // get route entry header
    struct rtmsg *rtp = (struct rtmsg *)NLMSG_DATA(nlp);
    // we only care about the tableId route table
    if (rtp->rtm_table != 254) continue;
    ...
    flags |= RTF_UP;
    if (gw != 0) flags |= RTF_GATEWAY;
    if (mask == (u32)(-1)) flags |= RTF_HOST;
